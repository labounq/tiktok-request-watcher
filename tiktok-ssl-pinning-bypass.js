...
function hook_SSL_CTX_set_custom_verify(library) {
    ...
    try {
        // Mise à jour pour Frida 17+
        const f = Process.getModuleByName(library.name).getExportByName(functionName);
...
// Hook natif désactivé : les 3 hooks Java ci-dessous (TrustManager custom,
// checkTrustedRecursive, SSL_CTX_set_custom_verify) suffisent déjà à voir
// le trafic en clair dans mitmproxy/Burp. Pas besoin de l'offset natif
// spécifique à libsscronet.so.
